...
def bookmarks():
    if not session.get("username"): return redirect(url_for("home"))
    conn = get_db()
    posts = conn.execute("SELECT p.*, u.is_verified FROM posts p JOIN bookmarks b ON p.id = b.post_id JOIN users u ON p.author = u.username WHERE b.user = ? ORDER BY b.id DESC", (session["username"],)).fetchall()
    conn.close()
...
def create_post():
    ...
    conn.execute("INSERT INTO posts (author, content, created_at) VALUES (?, ?, ?)", 
                 (session["username"], content, time_str))
    conn.commit(); conn.close()
...
def chat_api(partner):
    ...
    if request.method == "POST":
        ...
        if content:
            ...
            conn.execute("INSERT INTO notifications (recipient, sender, type, created_at) VALUES (?, ?, ?, ?)", (partner, me, "message", time_str))
            conn.commit()
            return jsonify({"success": True})
