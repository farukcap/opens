...
def init_db():
    conn = get_db()
    c = conn.cursor()
    # YENİ ÖZELLİKLER: streak, profile_views, mood eklendi
    c.execute("""CREATE TABLE IF NOT EXISTS users (
        username TEXT PRIMARY KEY, password TEXT, bio TEXT DEFAULT 'Mekan''da takılıyor.', 
    ...
    # YENİ ÖZELLİK: is_anonymous eklendi
    c.execute("""CREATE TABLE IF NOT EXISTS posts (
        id INTEGER PRIMARY KEY AUTOINCREMENT, author TEXT, content TEXT, 
        is_anonymous INTEGER DEFAULT 0, created_at TEXT, likes_count INTEGER DEFAULT 0, 
        retweets_count INTEGER DEFAULT 0, replies_count INTEGER DEFAULT 0, views_count INTEGER DEFAULT 0)""")
    ...
    if "username" in session:
        ...
        if user_data:
            ...
            if last_date != date_str:
                if last_date == (now - datetime.timedelta(days=1)).strftime("%Y-%m-%d"):
                    streak += 1
                elif last_date is not None:
                    streak = 1 # Seri bozuldu, baştan
                else:
                    streak = 1
        ...
        # Liderlik Tablosu (En çok takipçisi olanlar)
...
def home():
    ...
    conn = get_db()
    posts = conn.execute("SELECT p.*, u.is_verified, u.mood FROM posts p LEFT JOIN users u ON p.author = u.username ORDER BY p.id DESC LIMIT 50").fetchall()
    conn.close()
...
def profile(username):
    ...
    # Profil Görüntülenme Sayacı (Kendi profili değilse artır)
...
def notifications():
    if not session.get("username"): return redirect(url_for("home"))
    conn = get_db()
    notifs = conn.execute("SELECT n.*, u.is_verified FROM notifications n LEFT JOIN users u ON n.sender = u.username WHERE recipient = ? ORDER BY n.id DESC LIMIT 30", (session["username"],)).fetchall()
...
def messages_page():
    ...
    chat_list = []
    for c in chats:
        partner_data = conn.execute("SELECT is_verified, mood FROM users WHERE username = ?", (c['partner'],)).fetchone()
        chat_list.append({
            "partner": c['partner'], 
            "is_verified": partner_data['is_verified'] if partner_data else 0,
            "mood": partner_data['mood'] if partner_data else ''
        })
    conn.close()
...
def bookmarks():
    if not session.get("username"): return redirect(url_for("home"))
    conn = get_db()
    posts = conn.execute("SELECT p.*, u.is_verified FROM posts p JOIN bookmarks b ON p.id = b.post_id LEFT JOIN users u ON p.author = u.username WHERE b.user = ? ORDER BY b.id DESC", (session["username"],)).fetchall()
    conn.close()
...
def update_mood():
    ...
    mood = request.form.get("mood", "✨ Yeni")
    conn = get_db()
    conn.execute("UPDATE users SET mood = ? WHERE username = ?", (mood, session["username"]))
    conn.commit(); conn.close()
...
def create_post():
    ...
    is_anon = 1 if request.form.get("is_anonymous") == "true" else 0
    ...
    conn.execute("INSERT INTO posts (author, content, is_anonymous, created_at) VALUES (?, ?, ?, ?)", 
                 (session["username"], content, is_anon, time_str))
    conn.commit(); conn.close()
...
def chat_api(partner):
    ...
    if request.method == "POST":
        ...
        if content:
            ...
            # Anlık Bildirim Tetikle
            conn.execute("INSERT INTO notifications (recipient, sender, type, created_at) VALUES (?, ?, ?, ?)", 
                         (partner, me, "message", time_str))
            conn.commit()
            return jsonify({"success": True})
    ...
    partner_data = conn.execute("SELECT is_verified FROM users WHERE username = ?", (partner,)).fetchone()
    conn.close()
    
    # DM kutusu için JSON uyuşmazlığını düzelten mükemmel format
...
def post_action(action_type, post_id):
    ...
    if conn.execute(f"SELECT 1 FROM {table} WHERE user = ? AND post_id = ?", (session["username"], post_id)).fetchone():
        ...
        conn.execute(f"UPDATE posts SET {col} = {col} + 1 WHERE id = ?", (post_id,))
        if action_type == "like": 
            post = conn.execute("SELECT author, is_anonymous FROM posts WHERE id = ?", (post_id,)).fetchone()
            if post and post["is_anonymous"] == 0:
                notify(post["author"], session["username"], "like", post_id)
