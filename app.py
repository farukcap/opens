...
app = Flask(__name__)
app.secret_key = "bizim-mekan-v5-pro"
...
def init_db():
    conn = get_db()
    c = conn.cursor()
    # YENİ: is_verified eklendi (Mavi Tik)
    c.execute("""CREATE TABLE IF NOT EXISTS users (
        username TEXT PRIMARY KEY, password TEXT, bio TEXT DEFAULT 'Merhaba, ben Mekan''dayım!', 
        last_seen TEXT, is_verified INTEGER DEFAULT 0, created_at TEXT DEFAULT CURRENT_TIMESTAMP)""")
    ...
    # Kurucuyu Mavi Tikli (1) olarak oluştur
    if not c.execute("SELECT * FROM users WHERE username = ?", (ADMIN_USERNAME,)).fetchone():
        c.execute("INSERT INTO users (username, password, bio, is_verified) VALUES (?, ?, ?, 1)", 
...
def inject_global_data():
    if "username" in session:
        ...
        new_users = conn.execute("SELECT username, bio, is_verified FROM users WHERE username != ? ORDER BY created_at DESC LIMIT 5", (session["username"],)).fetchall()
        trends = conn.execute("SELECT tag, count FROM hashtags ORDER BY count DESC LIMIT 5").fetchall()
        
        # Çevrimiçi bilgisini güncelle
        conn.execute("UPDATE users SET last_seen = ? WHERE username = ?", (datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), session["username"]))
        conn.commit()
        
        # Kullanıcının kendi bilgilerini al (Mavi tik vs. için)
        current_user_data = conn.execute("SELECT is_verified FROM users WHERE username = ?", (session["username"],)).fetchone()
        conn.close()
        
        return dict(
            current_user=session["username"], 
            current_user_verified=current_user_data['is_verified'] if current_user_data else 0,
...
def home():
    ...
    conn = get_db()
    # Postları ve yazarların mavi tik durumunu çek
    posts = conn.execute("SELECT p.*, u.last_seen, u.is_verified FROM posts p JOIN users u ON p.author = u.username ORDER BY p.id DESC LIMIT 50").fetchall()
    conn.close()
...
def profile(username):
    ...
    posts = conn.execute("SELECT p.*, u.last_seen, u.is_verified FROM posts p JOIN users u ON p.author = u.username WHERE author = ? ORDER BY p.id DESC", (username,)).fetchall()
...
def messages_page():
    ...
    # Partnerlerin mavi tik durumunu da alalım
    chat_list = []
    for c in chats:
        partner_data = conn.execute("SELECT is_verified FROM users WHERE username = ?", (c['partner'],)).fetchone()
        chat_list.append({"partner": c['partner'], "is_verified": partner_data['is_verified'] if partner_data else 0})
...
def chat_api(partner):
    ...
    return jsonify({
        ...
        "partner_verified": partner_data['is_verified'] if partner_data else 0
    })

# Diğer aksiyonlar (Beğeni, RT, Follow vs.) öncekiyle aynı mantıkta. 
# Çok uzamaması ve "su gibi akması" için temel API'leri tutuyoruz.
