...
if action == "like":
    try:
        conn.execute("INSERT INTO likes (user, post_id, created_at) VALUES (?, ?, ?)",
                    (username, post_id, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        ...
        if post and post['author'] != username:
            conn.execute("INSERT INTO notifications (recipient, sender, type, post_id, created_at) VALUES (?, ?, 'like', ?, ?)",
                        (post['author'], username, post_id, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    ...
    try:
        conn.execute("INSERT INTO retweets (user, post_id, created_at) VALUES (?, ?, ?)",
                    (username, post_id, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
...
try:
    conn.execute("INSERT INTO follows (follower, followed, created_at) VALUES (?, ?, ?)",
                (username, target, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    ...
    conn.execute("INSERT INTO notifications (recipient, sender, type, created_at) VALUES (?, ?, 'follow', ?)",
                (target, username, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    conn.commit()
except:
...
if user and check_password_hash(user["password"], p):
    session["username"] = u
    conn.execute("UPDATE users SET last_login_date = ? WHERE username = ?",
...
conn = get_db()
conn.execute("UPDATE users SET bio = ?, mood = ?, avatar_color = ? WHERE username = ?",
            (bio, mood, avatar_color, username))
conn.commit()
conn.close()
